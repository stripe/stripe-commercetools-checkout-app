...
- **Documentation**: Express Checkout docs (enabler README—including session/cancel flow and `ExpressComponent` surface—processor `GET /payments` session note, JSDoc, changelog) updated for **commercetools Checkout Express** integration (`GET /express-payment-data`, `POST /express-config`). Framing uses commercetools Checkout / Connect only; removed wording that implied copying another PSP template or comparing to other PSPs.
