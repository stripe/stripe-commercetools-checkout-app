...
- **PI-First payment flow (Blik support / TOCTOU mitigation)**: New `STRIPE_PAYMENT_FLOW` environment variable (`deferred` | `pi_first`, default `deferred`). In `pi_first` mode the processor creates the PaymentIntent eagerly and returns `flowType: 'pi_first'` from `GET /config-element/payment`, suppressing `setupFutureUsage`; the enabler initializes Stripe Elements with `clientSecret` instead of `{ mode, amount, currency }`, which is required for payment methods (e.g. Blik) that must bind to a PaymentIntent before rendering. See `context/decisions/adr-006-pi-first-blik-toctou.md` for the full design, TOCTOU/orphan-PI tradeoffs, and the storefront contract for `requires_action`/Blik authorization.
- **Per-cart payment behavior overrides**: New `STRIPE_PAYMENT_BEHAVIOR_RULES` environment variable — a JSON map keyed by ISO country code or commercetools store key that overrides `captureMethod`, `flowType`, `setupFutureUsage`, and `collectBillingAddress` for matched carts. Resolved via the new `resolvePaymentBehavior()` / `extractDiscriminator()` helpers in `processor/src/services/payment-behavior-resolver.ts`. Flat env vars remain the default; malformed JSON aborts startup.
- **`STRIPE_BEHAVIOR_PAYMENT_ELEMENT`**: New optional environment variable to configure `elements.create('payment', options)` (Elements Behavior) — `terms`, `wallets`, `defaultValues`, `fields`, `business`, `paymentMethodOrder`, `readOnly`, `layout`. Coexists with the legacy `STRIPE_LAYOUT`/`STRIPE_COLLECT_BILLING_ADDRESS` variables: values set here take priority per attribute, with unset attributes falling back to those. Invalid JSON or unknown/invalid keys are dropped individually rather than failing the whole configuration.
