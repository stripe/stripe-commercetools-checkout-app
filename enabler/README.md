...
1. Checkout calls `enabler.createExpressBuilder('dropin')` and uses the builder to create and mount the Express component with options (callbacks, initial amount). On mount, the component only renders the Express button(s); no session is requested yet. The enabler sets amount and currency on the shared Stripe **Elements** instance from `ExpressOptions.initialAmount` (the Express Checkout element itself does not carry amount/currency in its options). The public **`ExpressComponent`** surface is **`mount` only**—session lifecycle is driven by callbacks, not by extra methods on the component.
